 ...
 *
 * Minimal adaptation:
 * - Avoids `@cardano-sdk/util-rxjs` dependency by replacing `toEmpty` with `ignoreElements()`.
 */
...
export type LoadedTxHistory = {
  transactions: Cardano.HydratedTx[];
  mightHaveMore: boolean;
};

export type TxHistoryLoader = {
  loadMore: () => void;
  retry: () => void;
...
export const createTxHistoryLoader = (
  ...
  const initialPage$ = concat(
    isSettled$.pipe(filter(Boolean), take(1), ignoreElements()),
