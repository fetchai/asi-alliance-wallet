...
import { BlockfrostClient, BlockfrostToCore } from '@cardano-sdk/cardano-services-client';
...
export class BlockfrostInputResolver implements Cardano.InputResolver {
  ...
  public async resolveInput(input: Cardano.TxIn, options?: Cardano.ResolveOptions): Promise<Cardano.TxOut | null> {
    ...
    try {
      ...
      if (response && response.outputs && response.outputs.length > input.index) {
      ...
      if (error?.status === NOT_FOUND_STATUS) {
        this.#logger.debug(`Input ${input.txId}#${input.index} not found on Blockfrost`);
        return null;
      }
      this.#logger.error(`Error resolving input ${input.txId}#${input.index}:`, error);
      throw error;
    }
