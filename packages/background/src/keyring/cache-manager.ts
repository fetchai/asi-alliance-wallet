...
export class AddressCacheManager {
      ...
      .then(async () => {
        const timeoutPromise = new Promise<never>((_, reject) => {
          timeoutId = setTimeout(() => {
            reject(new Error(`Lock timeout for ${lockKey} after ${AddressCacheManager.LOCK_TIMEOUT_MS}ms`));
   ...
   */
  private async withRetry<T>(fn: () => Promise<T>, retryDelayMs = 100): Promise<T> {
    try {
      return await fn();
    } catch (e: unknown) {
  ...
  private isEncryptedCacheData(data: any): boolean {
    ...
    try {
      ...
      return (
        parsed?.version === '1.0' &&
        c &&
        typeof c.cipher === 'string' && c.cipher === 'aes-128-ctr' &&
  ...
  private async decryptCacheData(encryptedData: string): Promise<CacheData> {
    ...
    try {
      ...
      const decrypted = await Crypto.decryptBlob(this.crypto, encrypted, this.password);
   ...
   */
  private async _loadCardanoCacheUnsafe(chainId: string): Promise<Record<string, { address: string; pubKey: string }>> {
    const key = this.getCardanoCacheKey(chainId);
    const data = await this.kvStore.get<string | Record<string, { address: string; pubKey: string }>>(key);
   ...
   */
  async loadCardanoCache(chainId: string): Promise<Record<string, { address: string; pubKey: string }>> {
    return this.withRetry(() => this.withLock(`cardano:${chainId}`, () => this._loadCardanoCacheUnsafe(chainId)));
  }

  /**
   * Internal: Save Cardano cache without lock
   */
  private async _saveCardanoCacheUnsafe(chainId: string, cache: Record<string, { address: string; pubKey: string }>): Promise<void> {
    const key = this.getCardanoCacheKey(chainId);
   ...
   */
  async saveCardanoCache(chainId: string, cache: Record<string, { address: string; pubKey: string }>): Promise<void> {
    return this.withRetry(() => this.withLock(`cardano:${chainId}`, () => this._saveCardanoCacheUnsafe(chainId, cache)));
  }

  /**
   * Internal: Load Generic cache without lock
   */
  private async _loadGenericCacheUnsafe(chainId: string): Promise<Record<string, { address: string; name?: string; pubKey?: string; mnemonicLength?: string }>> {
    const key = this.getGenericCacheKey(chainId);
    const data = await this.kvStore.get<string | Record<string, { address: string; name?: string; pubKey?: string; mnemonicLength?: string }>>(key);
    ...
    if (this.isEncryptedCacheData(data)) {
      ...
      try {
        const decrypted = await this.decryptCacheData(data as string);
        const result: Record<string, { address: string; name?: string; pubKey?: string; mnemonicLength?: string }> = {};
        for (const [walletId, entry] of Object.entries(decrypted)) {
          result[walletId] = {
            address: entry.address,
            name: entry.name,
    ...
    return data as Record<string, { address: string; name?: string; pubKey?: string; mnemonicLength?: string }>;
  }

  /**
   * Load Generic cache for specific chain
   */
  async loadGenericCache(chainId: string): Promise<Record<string, { address: string; name?: string; pubKey?: string; mnemonicLength?: string }>> {
    return this.withRetry(() => this.withLock(`generic:${chainId}`, () => this._loadGenericCacheUnsafe(chainId)));
  }

  /**
   * Internal: Save Generic cache without lock
   */
  private async _saveGenericCacheUnsafe(chainId: string, cache: Record<string, { address: string; name?: string; pubKey?: string; mnemonicLength?: string }>): Promise<void> {
    const key = this.getGenericCacheKey(chainId);
   ...
   */
  async saveGenericCache(chainId: string, cache: Record<string, { address: string; name?: string; pubKey?: string; mnemonicLength?: string }>): Promise<void> {
    return this.withRetry(() => this.withLock(`generic:${chainId}`, () => this._saveGenericCacheUnsafe(chainId, cache)));
  }
  ...
  async checkConsistency(
    ...
    return this.withRetry(() => this.withLock(lockKey, async () => {
      const issues: string[] = [];
      ...
      try {
        ...
        if (cacheIds.length !== walletIds.length) {
          issues.push(`Wallet count mismatch: cache has ${cacheIds.length}, expected ${walletIds.length}`);
        }
        ...
        for (let i = 0; i < walletIds.length; i++) {
          ...
          if (cachedEntry && 'name' in cachedEntry) {
            ...
            if (cachedName && cachedName !== expectedName) {
              issues.push(`Name mismatch for wallet ${id}: cached "${cachedName}", expected "${expectedName}"`);
            }
          }
        }
        ...
        if (activeWalletId && cache[activeWalletId]) {
          ...
          if (cachedAddr !== activeWalletAddress) {
            issues.push(`Active wallet address mismatch: cached "${cachedAddr.slice(0, 10)}...", expected "${activeWalletAddress.slice(0, 10)}..."`);
          }
    ...
    try {
      await Promise.all(
        this.embedChainInfos.map(async (info) => {
          ...
          const lockKey = isCardano ? `cardano:${info.chainId}` : `generic:${info.chainId}`;
   ...
   */
  async addWalletToAllCaches(walletId: string, walletName: string): Promise<void> {
    if (!walletId) return;
    ...
    try {
      ...
      console.error(`[AddressCacheManager] Wallet addition to cache failed:`, e);
  ...
  async removeWalletFromAllCaches(walletId: string): Promise<void> {
    ...
    try {
      ...
      console.error(`[AddressCacheManager] Wallet removal from cache failed:`, e);
      // Continue execution - cache operations are not critical for core functionality
    }
  }
      ...
      for (const info of this.embedChainInfos) {
        if (info?.features?.includes("cardano")) {
          ...
          if (data && !this.isEncryptedCacheData(data)) {
            ...
            await this.saveCardanoCache(chainId, data as Record<string, { address: string; pubKey: string }>);
            const loaded = await this.loadCardanoCache(chainId);
            if (!this.compareCardanoCaches(data as Record<string, { address: string; pubKey: string }>, loaded)) {
              await this.kvStore.set(key, data);
              await this.kvStore.set(`cache_migration_review_needed:${chainId}`, true as any);
            }
          }
        } else {
          const chainId = info.chainId;
          ...
          if (data && !this.isEncryptedCacheData(data)) {
            ...
            await this.saveGenericCache(chainId, data as Record<string, { address: string; name?: string }>);
            const loaded = await this.loadGenericCache(chainId);
            if (!this.compareGenericCaches(data as Record<string, { address: string; name?: string }>, loaded)) {
              await this.kvStore.set(key, data);
              await this.kvStore.set(`cache_migration_review_needed:${chainId}`, true as any);
            }
          }
  ...
  private compareCardanoCaches(a: Record<string, { address: string; pubKey: string }>, b: Record<string, { address: string; pubKey: string }>): boolean {
  ...
  private compareGenericCaches(a: Record<string, { address: string; name?: string }>, b: Record<string, { address: string; name?: string }>): boolean {
    const aKeys = Object.keys(a).sort();
    const bKeys = Object.keys(b).sort();
    if (aKeys.length !== bKeys.length) return false;
    for (let i = 0; i < aKeys.length; i++) if (aKeys[i] !== bKeys[i]) return false;
