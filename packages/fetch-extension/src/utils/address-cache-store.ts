...
class AddressCacheStore implements CacheManager {
  ...
  private async withLock<T>(
    ...
    const newLock = existingLock
      .then(async () => {
        const timeoutPromise = new Promise<never>((_, reject) => {
          timeoutId = setTimeout(() => {
            reject(
              new Error(
                `Lock timeout for ${lockKey} after ${AddressCacheStore.LOCK_TIMEOUT_MS}ms`
              )
            );
      ...
      .catch((error) => {
        console.error(
          `[AddressCacheStore] Lock operation failed for ${lockKey}:`,
          error
        );
    ...
    const newLockVoid = newLock
      .then(() => {})
      .catch((error) => {
        // Lock cleanup failed - log but don't throw to avoid breaking the operation
        console.error(
          `[AddressCacheStore] Lock cleanup failed for ${lockKey}:`,
          error
        );
      });
  ...
  setCache(chainId: string, data: AddressCacheById): void {
    this.withLock(chainId, () => {
      ...
      console.error(
        `[AddressCacheStore] setCache failed for ${chainId}:`,
        error
      );
   ...
   */
  private _updateCacheUnsafe(
    chainId: string,
    updates: Partial<AddressCacheById>
  ): void {
  ...
  updateCache(chainId: string, updates: Partial<AddressCacheById>): void {
    this.withLock(chainId, () => {
      ...
      console.error(
        `[AddressCacheStore] updateCache failed for ${chainId}:`,
        error
      );
      // Continue execution - cache operations are not critical
    });
  ...
  clearChainCache(chainId: string): void {
    this.withLock(chainId, () => {
      ...
      console.error(
        `[AddressCacheStore] clearChainCache failed for ${chainId}:`,
        error
      );
