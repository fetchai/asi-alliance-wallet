...
export const normalizeCacheData = (existingCache: AddressCacheById, currentIds: string[]): AddressCacheById => {
...
export const hasRequiredAddresses = (
  ...
  const result = requiredIds.every(id => {
    ...
    console.log(`[hasRequiredAddresses] Wallet ${id}: hasAddress=${hasAddress}, address="${cacheData[id] || 'EMPTY'}"`);
    return hasAddress;
  });
  console.log(`[hasRequiredAddresses] Result: ${result} for ${requiredIds.length} wallets`);
  return result;
};
