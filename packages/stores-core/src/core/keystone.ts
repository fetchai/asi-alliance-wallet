...
export class KeystoneStore {
  ...
  *resolveGetPubkey(data: unknown) {
    const datas = this.interactionStore.getAllData(TYPE_KEYSTONE_GET_PUBKEY);
    if (datas.length !== 0) {
  ...
  *resolveSign(data: unknown) {
    const datas = this.interactionStore.getAllData(TYPE_KEYSTONE_SIGN);
    if (datas.length !== 0) {
      yield this.interactionStore.approve(
        TYPE_KEYSTONE_SIGN,
        datas[0].id,
  ...
  get signData() {
    const datas =
      this.interactionStore.getAllData<SignData>(TYPE_KEYSTONE_SIGN);
    if (datas.length === 0) {
      return undefined;
    }
    return datas[0];
