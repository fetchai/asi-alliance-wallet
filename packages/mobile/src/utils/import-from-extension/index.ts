...
export interface QRCodeSharedData {
  ...
  wcURI: string;
  // Session-bound identifier generated by extension.
  sessionId: string;
  // One-time token that must match extension-side session.
  requestToken: string;
...
export function parseQRCodeDataForImportFromExtension(
  ...
  if (
    !sharedData.wcURI ||
    !sharedData.sharedPassword ||
    !sharedData.sessionId ||
    !sharedData.requestToken
  ) {
        ...
        params: [
          {
            ...
            sessionId: sharedData.sessionId,
            requestToken: sharedData.requestToken,
          },
        ],
      })
