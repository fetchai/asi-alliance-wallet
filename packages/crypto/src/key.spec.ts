...
describe("Test priv key", () => {
  ...
  it("priv key should generate the valid signature", () => {
    ...
    const data = new Uint8Array([1, 2, 3]);
    const signature = privKey.signDigest32(Hash.sha256(data));
    expect(signature).toStrictEqual(privKey.signDigest32(data));

    expect(pubKey.verifyDigest32(data, new Uint8Array(64))).toBe(true);
    expect(pubKey.verifyDigest32(Hash.sha256(data), new Uint8Array(64))).toBe(
      true
    );
  });
